...
class Highlighter < Redcarpet::Render::HTML
  def self.reload!
    if Object.const_defined? 'SPBFrontendHighlighter'
      Object.send(:remove_const, 'SPBFrontendHighlighter')
    end
  end
...
class Builder
  ...
  def markdown
    @renderer ||= begin
      load ROOT.join('spbfrontend_highlighter.rb').to_s
      Redcarpet::Markdown.new(SPBFrontendHighlighter.new, fenced_code_blocks: true)
    end
  end

  def sections
    @sections ||= ROOT.join('spbfrontend.md').read
...
task :build => :clean do
  html = ROOT.join('./build/spbfrontend.html')
  html.dirname.mkpath
